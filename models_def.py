...
class res_block(nn.Module):
    def __init__(self, num_neurons: int = 544, use_batchnorm: bool = False):
...
class DepthAngleEstimator_transformer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.res_pose1 = res_block(use_batchnorm=False)
        self.res_pose2 = res_block(use_batchnorm=False)
        self.res_angle1 = res_block(use_batchnorm=False)
        self.res_angle2 = res_block(use_batchnorm=False)
        self.depth = nn.Linear(544, num_joints)
        self.angles = nn.Linear(544, 1)
    ...
    def forward(self, x):
        x = x.view(-1, self.num_frame, 2, 17).permute(0, 1, 3, 2) #(256, 9, 17, 2)
        x = x.permute(0, 3, 1, 2)
        b, _, _, p = x.shape
